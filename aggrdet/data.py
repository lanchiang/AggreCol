...
def normalize_file(file: np.ndarray, number_format: str):
    ...
    for index, value in np.ndenumerate(trans_file):
        ...
        if str(index[0]) not in numeric_line_indices[0]:
            numeric_line_indices[0][str(index[0])] = []
        if str(index[1]) not in numeric_line_indices[1]:
            numeric_line_indices[1][str(index[1])] = []
        if matches is not None:
            numeric_line_indices[0][str(index[0])].append(index[1])

            numeric_line_indices[1][str(index[1])].append(index[0])
...
def detect_number_format(file: np.ndarray, sample=1) -> str:
    """
    ...
    # Occurrence of each format in the Troy dataset. We use it to break ties
    format_occurrences = {
        'Comma Point': 133/200,
        'Point Comma': 0/200,
        'Space Point': 12/200,
    ...
    max_count = len(max(formats2values.values(), key=lambda x: len(x)))
    formats2values = {k: v for k, v in formats2values.items() if len(v) == max_count}
    formats2values = {k: v for k, v in sorted(formats2values.items(), key=lambda x: format_occurrences.get(x[0]), reverse=True)}
    return [list(formats2values.keys())[0]]
